...
def detect(save_img=False):
    out, source, weights, view_img, save_txt, imgsz, facialFeatureLog = \
        opt.output, opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, opt.facialFeature
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if (ear_count == 2) and (eye_count == 2) and (nose_count == 1):
                ...
                recordBoxInfo(r['rois'], r['class_name'], img_name, score)
                xml.saveFile()
                # for c in det[:, -1].unique():
